...
class ModManager {
    ...
    getAllGifs(){//Récupération dans la table gifs des éléments gifId, title, url et date
        let sql = "SELECT gifs.id, gifs.title, gifs.name, gifs.url, DATE_FORMAT(DATE(gifs.date), '%d/%m/%Y') AS date, TIME(gifs.time) AS time FROM gifs ORDER BY gifs.date DESC";
        return new Promise((resolve) =>{
            connectdb.query(sql, function (err, result, fields) {
                if (err) throw err;
                resolve(result)
